...
def prepare_data(args):
    ...
    #
    source = source.replace('_train', "") if '_train' in source else source
    target = target.replace('_train', "") if '_train' in target else target
    ...
    with open(os.path.join(args.data_root, cfg.DATASET.DATAROOT, 'category.txt'), 'r') as f:
        classes = f.readlines()
    ...
    dataloaders[source] = CustomDatasetDataLoader(
                ...
                classnames=classes)
    # class-agnostic target dataloader for Self-paced training
    batch_size = cfg.TRAIN.TARGET_BATCH_SIZE
    dataset_type = 'SingleDataset'#'SingleDatasetWithoutLabel'
    ...
    test_domain=test_domain.replace('_test',"") if '_test' in test_domain else test_domain
    dataroot_test = os.path.join(args.data_root, cfg.DATASET.DATAROOT, test_domain)
